...
class _DateCourseState extends State<DateCourse> {
  ...
  Widget build(BuildContext context) {
    return Observer(builder: (BuildContext context) {
      if (context.read<TimetableStore>().loadOperation.status ==
                        ...
                        children: [
                          FittedBox(
                              child: Text(kday[now.weekday]!,
                                  style:
                                      MyFonts.w500.size(20).setColor(kWhite))),
                          FittedBox(
                              child: Text(
                            now.day.toString(),
            ...
            (show)
                ? (now.hour < 14)
                    ? ListView.builder(
                        ...
                        itemCount: context
                            .read<TimetableStore>()
                            .todayTimeTableMorning
                            .length,
                        itemBuilder: (context, index) => context
                            .read<TimetableStore>()
                            .todayTimeTableMorning[index])
                    : ListView.builder(
                        shrinkWrap: true,
                        physics: ClampingScrollPhysics(),
                        itemCount: context
                            .read<TimetableStore>()
                            .todayTimeTableAfternoon
                            .length,
                        itemBuilder: (context, index) => context
                            .read<TimetableStore>()
                            .todayTimeTableAfternoon[index])
                : SizedBox(),
          ],
        );
      } else {
